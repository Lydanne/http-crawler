...
class Directive {
     ...
     */
    ['v-refer'](refer, key) {
        return jsonpath.query(refer, key);
    }
    /**
     * 根据key返回状态的值
     * @param key JSON查询字符串
     */
    ['v-state'](refer, key) {
        return jsonpath.query(refer.state, key);
    }
    /**
     * 根据key返回状态的值
     * @param key JSON查询字符串
     */
    ['v-meta'](refer, key) {
        return jsonpath.query(refer.meta, key);
    }
    /**
     * 查询上一个step的对象
     * @param key JSON查询字符串
     */
    ['v-prev'](refer, key) {
        return jsonpath.query(refer.steps[refer.state.current - 1], key);
    }
    /**
    ...
    ['v-prev-mres'](refer, key) {
        return jsonpath.query(refer.steps[refer.state.current - 1].mergeResults, key);
    }
    /**
    ...
    ['v-prev-res'](refer, key) {
        return jsonpath.query(refer.steps[refer.state.current - 1].results, key);
    }
    /**
    ...
    ['v-prev-responses'](refer, key) {
        return jsonpath.query(refer.steps[refer.state.current - 1].responses, key);
    }
    /**
     * 查询当前step的对象
     * @param key JSON查询字符串
     */
    ['v-current'](refer, key) {
        return jsonpath.query(refer.steps[refer.state.current], key);
    }
    /**
     * 获取当前的response
     * @param key JSON查询字符串
    ...
    ['v-response'](refer, key) {
        return jsonpath.query(refer.response, key);
    }
    /**
     * 获取当前的response
     * @param key JSON查询字符串
    ...
    ['v-response-html'](refer, key) {
        ...
        return jsonpath.query(result, jsonSelector);
    }
}
...
class HttpCrawler {
    ...
    async go(meta) {
        ...
        this.event.emit(exports.EventList.GO_BEFORE, this);
        ...
        for (let i = 0; i < currentStep.requests.length; i++) {
            ...
            do {
                try {
                    ...
                    errRetry--;
                    isRetry = true;
                    this.event.emit(exports.EventList.ERR, error, this);
                }
        ...
        this.event.emit(exports.EventList.GO_AFTER, this);
        this.state.current++;
        ...
        if (this.state.current === this.steps.length) {
            this.event.emit(exports.EventList.END, this);
        }
    ...
    splitFull(transformValue) {
        ...
        while (t._v.length > 0) {
            ...
            for (let i = 0; i < t._v.length; i++) {
                const cur = t._v[i];
                jsonpath.apply(_t, cur, (value) => {
                    if (!isArray(value)) {
                        return value;
                    }
                    return value[0];
                });
                jsonpath.apply(t, cur, (value) => {
                    if (!isArray(value)) {
                        t._v.splice(i, 1);
                        return value;
                    }
